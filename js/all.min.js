...
class Connection {
    ...
    constructor(x_start, y_start, x_end, y_end, connection_count, direction) {
    ...
    init() {
        ...
        var mark_function = function(chip) {
			...
			if (marked_chips.length <= 0 || (marked_chips.length == 1 && marked_chips.indexOf(chip) >= 0)) {
				...
				if (first_chip.x === chip.x) {
                    var direction = "ve";
                    var x_min = first_chip.x;
                    var x_max = first_chip.x;
                    var y_min = Math.min(first_chip.y, chip.y) + 1;
                    var y_max = Math.max(first_chip.y, chip.y) - 1;
                } 
                else {
                    var direction = "ho";
                    var x_min = Math.min(first_chip.x, chip.x) + 1;
                    var x_max = Math.max(first_chip.x, chip.x) - 1;
                    var y_min = first_chip.y;
                    var y_max = first_chip.y;
                }

				
				var connection = new Connection(x_min, y_min, x_max, y_max, 1, direction);
