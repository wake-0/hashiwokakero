    ...
    load_from_csv() {
        ...
        Papa.parse(textarea.value, { delimiter: ' ',
            ...
            complete: function(data, file)
            {
                ...
                for (var row = 0; row < data.data.length; row++) {
                    var col = 0; 
                    while (col < data.data[row].length) {
                        var entry = data.data[row][col];
                        if (entry !== '*' && entry !== '+') {
                            col++;
                            continue;
                        ...
                        // try get first chip
                        if (col <= 0 || col >= data.data[row].length) {
                            alert("Parsing of the connections, was not successfull.");
                            return;
                        }
                        
                        var first = temp_chips.find(function(item) { return item.x === (col - 1) && item.y === row });
                        var symbol = entry;
                        
                        while (col < data.data[row].length && entry === symbol) {
                            entry = data.data[row][col];
                            col++;
                        }
                        
                        var second = temp_chips.find(function(item) { return item.x === (col - 1) && item.y === row });
